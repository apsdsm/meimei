...
repository = "jjc2-api"          # exact ECR repository name. Required.
...
profile = "jjc2-shared"          # empty means ambient credentials, which is what CI has
...
```
036558358799.dkr.ecr.ap-northeast-1.amazonaws.com/jjc2-api:sha-abc1234
└──────── registry ─────────────────────────────┘ └ repo ┘ └── tag ──┘
```
...
```
$ meimei build api
Error: repository "jjc2-api" does not exist in account 036558358799 (ap-northeast-1)
...
both from one variable, so `jjc2-dev1-internal` is both an ECS service name and a task definition
