...
and ECS deploy run against `jjc_manualchatbot` on 2026-08-19, and kept current with the code.
...
survive a deploy that in fact drops it. Both were observed on jjc2 dev1 on 2026-08-20.
...
**Verify a deploy from a genuinely different source address.** `jjc_manualchatbot` sits behind an ALB
