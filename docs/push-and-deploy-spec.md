...
`nova_chatbot` is the simplest possible ECS target, which is what makes it a good first case:
...
```
account   123456789012            region  ap-northeast-1     profile  tcr
ECR repo  nova-chatbot            IMMUTABLE tags, scanOnPush
cluster   nova-public1
service   nova-public1-chatbot    == the task definition family
container chatbot               port 3300, bridge mode, dynamic host port
platform  linux/arm64             t4g.small (Graviton)
```

Config was 16 lines of `.meimei.toml`. `project.name = "tc"` plus service `chatbot` gives
`nova-chatbot`, matching the repo Terraform created — the derivation needs no extra field.
...
```bash
aws sts get-caller-identity --query Account --output text     # 123456789012, matched
...
```bash
aws ecr describe-images --region ap-northeast-1 --repository-name nova-chatbot \
...
docker buildx build \
  ...
  --tag "$REGISTRY/nova-chatbot:sha-eba96de" \
  --load .

docker push "$REGISTRY/nova-chatbot:sha-eba96de"      # 61 MB, 9.9s
```

`--load` then `docker push`, not `--push`. This is `acme_main/scripts/build.sh`'s
...
```bash
TD=$(aws ecs describe-task-definition --task-definition nova-public1-chatbot --query taskDefinition)
...
# → arn:...:task-definition/nova-public1-chatbot:5
```
...
```bash
aws ecs update-service --cluster nova-public1 --service nova-public1-chatbot \
...
switch to `--load` + `docker push`. `acme_main/scripts/build.sh` decides this from the builder's
...
probe from the EC2 instance's own public IP (`203.0.113.10`, not allowlisted) returned the truth:
...
5. On failure, report **why**: the last STOPPED task's `stoppedReason` plus every container
   `reason`. `acme_main/scripts/deploy.sh:latest_stopped_reason` is the shape.
...
`acme_main/scripts/deploy.sh`'s projection lists eleven. A task using `ephemeralStorage`,
