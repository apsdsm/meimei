...
The need was hit on `jjc_manualchatbot`. `tc-public1` is one `t4g.small`; on ECS/EC2 the cost is the
...
`services` is a list because an environment is often packed across several ECS services. jjc2 dev1 is
