...
**Status:** not built. The need was hit on `nova_chatbot` on 2026-08-21; nothing was written
...
`nova-public1` is one `t4g.small` running the chatbot. On ECS/EC2 the cost is the instance, not the
task, so a second task on that box is free — the goal was a staging service beside production:

```
cluster nova-public1
  service nova-public1-chatbot        host chat.example.com        (production)
  service nova-public1-chatbot-test   host test.chat.example.com   (staging)
```

Both pull from **one** repository, `nova-chatbot`, because the point of staging is to run the exact
...
`nova-public1-chatbot` sorts before `nova-public1-chatbot-test`, so `byContainer["chatbot"]` ends up
...
gives it a distinct repository, `nova-chatbot-test`. That means a second build and a second push, which
...
2. **The repository is unaffected.** `chatbot` deployed to `public1-test` still pulls
   `nova-chatbot:<tag>`. This is the requirement; everything else is mechanism.
...
`nova-public1-chatbot-stg` runs a container called `chatbot`, the same name production's carries. It was
...
So the collision is live. `meimei deploy chatbot` against `nova-public1` will roll one of the two
...
target-aware, and it is documented as such in `nova_chatbot/docs/container.md`.
