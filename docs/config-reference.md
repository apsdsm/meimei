...
refuses every other one by name. Two config files exist in the wild (`jjc2_main`,
`jjc_manualchatbot`); both get migrated when 1.1.0 is tagged.
...
```
Error: /home/nick/Code/jjc2_main/.meimei.toml: this file is version 1 (it has no version key)
...
```
Error: /home/nick/Code/jjc2_main/.meimei.toml: this file is version 99, and this meimei reads version 2
...
Every key, with the multi-image and multi-target shape. `jjc_manualchatbot` is the same file with one
...
[project]
name     = "jjc2"                # prefixes every ECR repository: jjc2-api, jjc2-user-web-spa, …
...
name       = "user-web-spa"
short      = "as"
dockerfile = "services/web/jjc2_user_web_spa/Dockerfile"
...
| `cluster` | yes | — | ECS cluster name. Spelled out rather than derived, because the convention differs per product (`jjc2-dev1-ecs-cluster-dev1` against `tc-public1`). |
