...
(`acme_main`, `nova_chatbot`). Taking one breaking change instead of two costs one migration
...
- **`meimei deploy` prints `service` in both senses.** `no service named` means "you did not name an
  image"; `service is on acme-dev1-internal:5, deploying from :7` means the ECS service. One command,
...
| Repo | Edit |
|---|---|
| `acme_main/.meimei.toml` | `version = 2` as the first line; five `[[services]]` → `[[images]]`. Plus a prose pass: the header comment and the `# TARGETS #` block use "service" throughout in the old sense, including a paragraph that already has to say "an ECS service's name is its task family" in the AWS sense in the same sentence. |
| `nova_chatbot/.meimei.toml` | `version = 2` as the first line; one `[[services]]` → `[[images]]`. Header comment says "One service, one image, one ECR repo", which becomes one sentence shorter. |
...
| 8 | Migrate `acme_main` and `nova_chatbot`, after installing the tagged binary. |
...
**Step 0, with the installed 1.0.0 binary**, from `acme_main`:
...
Diff through a normaliser, because the default tag is derived from the commit and HEAD in these repos
moves for reasons that have nothing to do with meimei — it moved once mid-rename during this work:

```bash
norm() { sed -E 's/sha-[0-9a-f]{7,40}/sha-HEAD/g'; }
```

Without it a colleague's commit in `acme_main` reads as five changed lines in `ls --json` and looks
like a rename bug. A run with an explicit `--tag` needs no normalising, which is why the tagged
dry-run below is the check to trust.

**After each of steps 1-4**, with the newly built binary:

```bash
meimei --config .meimei.v2.toml ls --json | norm | diff <(norm < /tmp/before-ls.json) -
meimei --config .meimei.v2.toml deploy --all --to dev1 --tag acme.20260820.002 --dry-run 2>&1 |
  diff /tmp/before-deploy-tagged.txt -
```
...
| Command | Expected |
|---|---|
| `meimei ls` in `acme_main` (real file, still v1) | refused, naming the two v1 → v2 edits |
...
**After step 8**, from `nova_chatbot` with the real migrated file:
