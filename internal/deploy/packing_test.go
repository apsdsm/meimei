...
func TestScopedPackingSeparatesTwoEnvironmentsInOneCluster(t *testing.T) {
	prod, err := NewPacking([]Service{svc("nova-public1-chatbot", "chatbot")}, "nova-public1", true)
	if err != nil {
		t.Fatalf("prod: %v", err)
	}
	stg, err := NewPacking([]Service{svc("nova-public1-chatbot-stg", "chatbot")}, "nova-public1", true)
	if err != nil {
		t.Fatalf("stg: %v", err)
	}

	p, ok := prod.ServiceFor("chatbot")
	if !ok || p.Family != "nova-public1-chatbot" {
		t.Errorf("prod resolved to %v, want nova-public1-chatbot", p)
	}
	s, ok := stg.ServiceFor("chatbot")
	if !ok || s.Family != "nova-public1-chatbot-stg" {
		t.Errorf("stg resolved to %v, want nova-public1-chatbot-stg", s)
	}
}
...
func TestDuplicateContainerInOneScopeIsRefused(t *testing.T) {
	_, err := NewPacking([]Service{
		svc("nova-public1-chatbot", "chatbot"),
		svc("nova-public1-chatbot-stg", "chatbot"),
	}, "nova-public1", true)

	if err == nil {
		t.Fatal("two services carrying `chatbot` were accepted, want a refusal")
	}
	for _, want := range []string{"chatbot", "nova-public1-chatbot", "nova-public1-chatbot-stg"} {
...
func TestDuplicateWithNoScopeSaysToDeclareServices(t *testing.T) {
	...
	for _, want := range []string{"services", "[[targets]]", "nova-public1"} {
		if !strings.Contains(err.Error(), want) {
...
func TestOneScopeMayHoldSeveralServices(t *testing.T) {
	p, err := NewPacking([]Service{
		svc("acme-dev1-internal", "api", "process-runner"),
		svc("acme-dev1-external", "user-web-spa"),
	}, "acme-dev1", true)
	if err != nil {
	...
	for container, family := range map[string]string{
		"api":            "acme-dev1-internal",
		"process-runner": "acme-dev1-internal",
		"user-web-spa":   "acme-dev1-external",
	} {
...
func TestFamilyFromARN(t *testing.T) {
	cases := []struct{ in, want string }{
		{"arn:aws:ecs:ap-northeast-1:123456789012:task-definition/nova-public1-chatbot:7", "nova-public1-chatbot"},
