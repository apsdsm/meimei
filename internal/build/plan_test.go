...
func fixture(t *testing.T, toml string, dockerfiles ...string) *catalog.Catalog {
	...
	cat.Git = catalog.Git{
		...
		RemoteURL: "git@github.com:example/jjc2.git",
		Available: true,
	}
...
func TestResolveSingleService(t *testing.T) {
	...
	p := plans[0]
	if p.Image != "jjc2-api:sha-abc1234" {
		t.Errorf("Image = %q, want jjc2-api:sha-abc1234", p.Image)
	}
...
func TestResolveLabelTagsVerbatim(t *testing.T) {
	...
	if plans[0].Image != "jjc2-api:jjc2.2026_010.001" {
...
func TestOCILabels(t *testing.T) {
	...
	if !strings.Contains(got, "org.opencontainers.image.source=git@github.com:example/jjc2.git") {
...
func TestImageRefHonoursRegistryHost(t *testing.T) {
	...
	o := opts("api")
	o.Output = OutputPush
	o.RegistryHost = "629585638685.dkr.ecr.ap-northeast-1.amazonaws.com"
	plans, err := Resolve(cat, o)
	if err != nil {
		t.Fatal(err)
	}
	want := "629585638685.dkr.ecr.ap-northeast-1.amazonaws.com/jjc2-api:sha-abc1234"
	...
	if plans[0].Image != "jjc2-api:sha-abc1234" {
